...
class Poblacion_GROVITAL_Screen extends StatefulWidget {
  const Poblacion_GROVITAL_Screen({Key? key}) : super(key: key);

  @override
  _Poblacion_GROVITAL_Screen_State createState() =>
      _Poblacion_GROVITAL_Screen_State();
}

class _Poblacion_GROVITAL_Screen_State
    extends State<Poblacion_GROVITAL_Screen> {
  late final DatabaseReference _GROVITALRef;
  late final DatabaseReference _GROVITALBaseRef;
  ...
  void _fetchTerrenos() async {
    try {
      DatabaseEvent event = await _GROVITALBaseRef.once();
  ...
  void _updateTableRowsResult() {
    ...
    Future.delayed(const Duration(milliseconds: 300), () {
      if (isTableVisibleResult) {
        _scrollControllerGROVITAL.animateTo(
          _scrollControllerGROVITAL.position.maxScrollExtent,
...
// coach mark
  final ScrollController _scrollControllerGROVITAL = ScrollController();
  final GlobalKey _SelectPiscPOBLACIONGROVITAL = GlobalKey();
  final GlobalKey _BeforePoblacion = GlobalKey();
  final GlobalKey _AfterPoblacion = GlobalKey();
  final GlobalKey _DiametroAtarrayaPOBLACIONGROVITAL = GlobalKey();
  final GlobalKey _NumberLancesGROVITAL = GlobalKey();
  final GlobalKey _ResultFinalPoblacionGROVITAL = GlobalKey();
  final GlobalKey _resetTutorialPoblation = GlobalKey();
  TutorialCoachMark? tutorialCoachMarkPOBLACIONGROVITAL;
  ...
  void dispose() {
    ...
    _timerPOBLACIONGROVITAL?.cancel(); // Cancela el timer si está activo
    tutorialCoachMarkPOBLACIONGROVITAL?.finish();
    super.dispose();
  }

  String get tutorial_mostrado_GROVITAL_POBLACION =>
      "tutorial_mostrado_GROVITAL_POBLACION";

  // Verifica si el tutorial ya fue mostrado
  Future<bool> _shouldShowTutorialPOBLACIONGROVITAL() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getBool(tutorial_mostrado_GROVITAL_POBLACION) ??
        true; // Si no existe, se muestra
  }

// Guarda que el tutorial ya se mostró
  Future<void> _setTutorialShownPOBLACIONGROVITAL() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(tutorial_mostrado_GROVITAL_POBLACION, false);
  }

// Restablece el tutorial desde ajustes si el usuario lo quiere repetir
  Future<void> resetTutorialPOBLACIONGROVITAL() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(tutorial_mostrado_GROVITAL_POBLACION, true);
  }

  // Muestra el tutorial solo si no se ha mostrado antes
  void showTutorialPOBLACIONGROVITAL() async {
    if (await _shouldShowTutorialPOBLACIONGROVITAL()) {
      tutorialCoachMarkPOBLACIONGROVITAL = TutorialCoachMark(
        targets: _initTargetsPOBLACIONGROVITAL,
        ...
        pulseEnable: true,
        onFinish: () {
          _setTutorialShownPOBLACIONGROVITAL(); // Guarda que ya se mostró
        },
  ...
  final List<TargetFocus> _initTargetsPOBLACIONGROVITAL = [];
  ...
  void initState() {
    ...
    if (kIsWeb) {
      ...
      _GROVITALBaseRef =
          FirebaseDatabase.instance.ref(EnvLoader.get('GROVITAL_')!);
    } else {
      _GROVITALRef =
          FirebaseDatabase.instance.ref(dotenv.env['GROVITAL_ROWS']!);
      _GROVITALBaseRef =
          FirebaseDatabase.instance.ref(dotenv.env['GROVITAL_']!);
    }
    _initTargetsPOBLACIONGROVITAL.add(
      TargetFocus(
        identify: "SelectPiscina",
        keyTarget: _SelectPiscPOBLACIONGROVITAL,
    ...
    _initTargetsPOBLACIONGROVITAL.add(
      TargetFocus(
        identify: "NumberLancesGROVITAL",
        keyTarget: _NumberLancesGROVITAL,
        alignSkip: Alignment.bottomRight,
        radius: 20,
        shape: ShapeLightFocus.RRect,
        contents: [
    ...
    _initTargetsPOBLACIONGROVITAL.add(
      TargetFocus(
        ...
        keyTarget: _DiametroAtarrayaPOBLACIONGROVITAL,
        alignSkip: Alignment.bottomRight,
        radius: 20,
        shape: ShapeLightFocus.RRect,
        contents: [
    ...
    _initTargetsPOBLACIONGROVITAL.add(
      TargetFocus(
        ...
        keyTarget: _ResultFinalPoblacionGROVITAL,
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _timerPOBLACIONGROVITAL = Timer(const Duration(seconds: 1), () {
        if (mounted) {
          showTutorialPOBLACIONGROVITAL();
        }
      });
    });
  ...
  void _loadHectareasPiscinas(String finca) async {
    final snapshot = await _GROVITALRef.get();
    if (snapshot.exists) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        controller: _scrollControllerGROVITAL,
        child: Padding(
          padding: const EdgeInsets.all(8.0),
          child: Center(
            child: Column(
              children: [
                ...
                const SizedBox(height: 10),
                Padding(
                  key: _ResultFinalPoblacionGROVITAL,
                  padding: const EdgeInsets.all(8.0),
                  child: InkWell(
                    onTap: () {
      ...
      floatingActionButton: SpeedDial(
        ...
        children: [
          ...
          SpeedDialChild(
            ...
            onTap: () async {
              await resetTutorialPOBLACIONGROVITAL();
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text(
  ...
  Widget _buildHectSelect(List<String> itemsHect, String selectedItemHect,
    ...
    return Column(
      ...
      children: [
        Text(
          key: _SelectPiscPOBLACIONGROVITAL,
          titleHect,
          style: const TextStyle(
            fontSize: 18,
            fontWeight: FontWeight.bold,
  ...
  Widget CustomTextFieldDiametro(
    ...
    return Container(
      ...
      child: Padding(
        padding: const EdgeInsets.all(8.0),
        child: TextField(
          key: _DiametroAtarrayaPOBLACIONGROVITAL,
          controller: controller,
          keyboardType: keyboardType,
          readOnly: isReadOnly,
          decoration: InputDecoration(
