...
class Poblacion_CAMANOVILLO_Screen extends StatefulWidget {
  const Poblacion_CAMANOVILLO_Screen({Key? key}) : super(key: key);

  @override
  _Poblacion_CAMANOVILLO_Screen_State createState() =>
      _Poblacion_CAMANOVILLO_Screen_State();
}

class _Poblacion_CAMANOVILLO_Screen_State
    extends State<Poblacion_CAMANOVILLO_Screen> {
  late final DatabaseReference _CAMANOVILLORef;

  late final DatabaseReference _CAMANOVILLOBaseRef;
  ...
  Timer? _timerPOBLACIONCAMANOVILLO;
  ...
  void _fetchTerrenos() async {
    try {
      DatabaseEvent event = await _CAMANOVILLOBaseRef.once();
  ...
  void _updateTableRowsResult() {
    ...
    Future.delayed(const Duration(milliseconds: 300), () {
      if (isTableVisibleResult) {
        _scrollControllerCAMANOVILLO.animateTo(
          _scrollControllerCAMANOVILLO.position.maxScrollExtent,
...
// coach mark
  final ScrollController _scrollControllerCAMANOVILLO = ScrollController();
  final GlobalKey _SelectPiscPOBLACIONCAMANOVILLO = GlobalKey();
  final GlobalKey _BeforePoblacion = GlobalKey();
  final GlobalKey _AfterPoblacion = GlobalKey();
  final GlobalKey _DiametroAtarrayaPOBLACIONCAMANOVILLO = GlobalKey();
  final GlobalKey _NumberLancesCAMANOVILLO = GlobalKey();
  final GlobalKey _ResultFinalPoblacionCAMANOVILLO = GlobalKey();
  final GlobalKey _resetTutorialPoblation = GlobalKey();
  TutorialCoachMark? tutorialCoachMarkPOBLACIONCAMANOVILLO;
  ...
  void dispose() {
    ...
    _timerPOBLACIONCAMANOVILLO?.cancel(); // Cancela el timer si está activo
    tutorialCoachMarkPOBLACIONCAMANOVILLO?.finish();
    super.dispose();
  }

  String get tutorial_mostrado_CAMANOVILLO_POBLACION =>
      "tutorial_mostrado_CAMANOVILLO_POBLACION";

  // Verifica si el tutorial ya fue mostrado
  Future<bool> _shouldShowTutorialPOBLACIONCAMANOVILLO() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getBool(tutorial_mostrado_CAMANOVILLO_POBLACION) ??
        true; // Si no existe, se muestra
  }

// Guarda que el tutorial ya se mostró
  Future<void> _setTutorialShownPOBLACIONCAMANOVILLO() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(tutorial_mostrado_CAMANOVILLO_POBLACION, false);
  }

// Restablece el tutorial desde ajustes si el usuario lo quiere repetir
  Future<void> resetTutorialPOBLACIONCAMANOVILLO() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(tutorial_mostrado_CAMANOVILLO_POBLACION, true);
  }

  // Muestra el tutorial solo si no se ha mostrado antes
  void showTutorialPOBLACIONCAMANOVILLO() async {
    if (await _shouldShowTutorialPOBLACIONCAMANOVILLO()) {
      tutorialCoachMarkPOBLACIONCAMANOVILLO = TutorialCoachMark(
        targets: _initTargetsPOBLACIONCAMANOVILLO,
        ...
        pulseEnable: true,
        onFinish: () {
          _setTutorialShownPOBLACIONCAMANOVILLO(); // Guarda que ya se mostró
        },
  ...
  final List<TargetFocus> _initTargetsPOBLACIONCAMANOVILLO = [];
  ...
  void initState() {
    ...
    if (kIsWeb) {
      ...
      _CAMANOVILLORef =
          FirebaseDatabase.instance.ref(dotenv.env['CAMANOVILLO_ROWS']!);
      _CAMANOVILLOBaseRef =
          FirebaseDatabase.instance.ref(dotenv.env['CAMANOVILLO_']!);
    }
    _initTargetsPOBLACIONCAMANOVILLO.add(
      TargetFocus(
        identify: "SelectPiscina",
        keyTarget: _SelectPiscPOBLACIONCAMANOVILLO,
    ...
    _initTargetsPOBLACIONCAMANOVILLO.add(
      TargetFocus(
        identify: "NumberLancesCAMANOVILLO",
        keyTarget: _NumberLancesCAMANOVILLO,
        alignSkip: Alignment.bottomRight,
        radius: 20,
        shape: ShapeLightFocus.RRect,
        contents: [
    ...
    _initTargetsPOBLACIONCAMANOVILLO.add(
      TargetFocus(
        ...
        keyTarget: _DiametroAtarrayaPOBLACIONCAMANOVILLO,
        alignSkip: Alignment.bottomRight,
        radius: 20,
        shape: ShapeLightFocus.RRect,
        contents: [
    ...
    _initTargetsPOBLACIONCAMANOVILLO.add(
      TargetFocus(
        ...
        keyTarget: _ResultFinalPoblacionCAMANOVILLO,
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _timerPOBLACIONCAMANOVILLO = Timer(const Duration(seconds: 1), () {
        if (mounted) {
          showTutorialPOBLACIONCAMANOVILLO();
        }
      });
    });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        controller: _scrollControllerCAMANOVILLO,
        child: Padding(
          padding: const EdgeInsets.all(8.0),
          child: Center(
            child: Column(
              children: [
                ...
                const SizedBox(height: 10),
                Padding(
                  key: _ResultFinalPoblacionCAMANOVILLO,
                  padding: const EdgeInsets.all(8.0),
                  child: InkWell(
                    onTap: () {
      ...
      floatingActionButton: SpeedDial(
        ...
        children: [
          ...
          SpeedDialChild(
            ...
            onTap: () async {
              await resetTutorialPOBLACIONCAMANOVILLO();
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text(
  ...
  Widget _buildHectSelect(List<String> itemsHect, String selectedItemHect,
    ...
    return Column(
      ...
      children: [
        Text(
          key: _SelectPiscPOBLACIONCAMANOVILLO,
          titleHect,
          style: const TextStyle(
            fontSize: 18,
            fontWeight: FontWeight.bold,
  ...
  Widget CustomTextFieldDiametro(
    ...
    return Container(
      ...
      child: Padding(
        padding: const EdgeInsets.all(8.0),
        child: TextField(
          key: _DiametroAtarrayaPOBLACIONCAMANOVILLO,
          controller: controller,
          keyboardType: keyboardType,
          readOnly: isReadOnly,
          decoration: InputDecoration(
