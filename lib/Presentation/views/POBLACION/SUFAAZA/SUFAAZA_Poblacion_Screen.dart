...
class Poblacion_SUFAAZA_Screen extends StatefulWidget {
  const Poblacion_SUFAAZA_Screen({Key? key}) : super(key: key);

  @override
  _Poblacion_SUFAAZA_Screen_State createState() =>
      _Poblacion_SUFAAZA_Screen_State();
}

class _Poblacion_SUFAAZA_Screen_State extends State<Poblacion_SUFAAZA_Screen> {
  late final DatabaseReference _SUFAAZARef;

  late final DatabaseReference _SUFAAZABaseRef;
  ...
  void _fetchTerrenos() async {
    try {
      DatabaseEvent event = await _SUFAAZABaseRef.once();
  ...
  void _updateTableRowsResult() {
    ...
    Future.delayed(const Duration(milliseconds: 300), () {
      if (isTableVisibleResult) {
        _scrollControllerSUFAAZA.animateTo(
          _scrollControllerSUFAAZA.position.maxScrollExtent,
...
// coach mark
  final ScrollController _scrollControllerSUFAAZA = ScrollController();
  final GlobalKey _SelectPiscPOBLACIONSUFAAZA = GlobalKey();
  final GlobalKey _BeforePoblacion = GlobalKey();
  final GlobalKey _AfterPoblacion = GlobalKey();
  final GlobalKey _DiametroAtarrayaPOBLACIONSUFAAZA = GlobalKey();
  final GlobalKey _NumberLancesSUFAAZA = GlobalKey();
  final GlobalKey _ResultFinalPoblacionSUFAAZA = GlobalKey();
  final GlobalKey _resetTutorialPoblation = GlobalKey();
  TutorialCoachMark? tutorialCoachMarkPOBLACIONSUFAAZA;
  ...
  void dispose() {
    ...
    _timerPOBLACIONSUFAAZA?.cancel(); // Cancela el timer si está activo
    tutorialCoachMarkPOBLACIONSUFAAZA?.finish();
    super.dispose();
  }

  String get tutorial_mostrado_SUFAAZA_POBLACION =>
      "tutorial_mostrado_SUFAAZA_POBLACION";

  // Verifica si el tutorial ya fue mostrado
  Future<bool> _shouldShowTutorialPOBLACIONSUFAAZA() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getBool(tutorial_mostrado_SUFAAZA_POBLACION) ??
        true; // Si no existe, se muestra
  }

// Guarda que el tutorial ya se mostró
  Future<void> _setTutorialShownPOBLACIONSUFAAZA() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(tutorial_mostrado_SUFAAZA_POBLACION, false);
  }

// Restablece el tutorial desde ajustes si el usuario lo quiere repetir
  Future<void> resetTutorialPOBLACIONSUFAAZA() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(tutorial_mostrado_SUFAAZA_POBLACION, true);
  }

  // Muestra el tutorial solo si no se ha mostrado antes
  void showTutorialPOBLACIONSUFAAZA() async {
    if (await _shouldShowTutorialPOBLACIONSUFAAZA()) {
      tutorialCoachMarkPOBLACIONSUFAAZA = TutorialCoachMark(
        targets: _initTargetsPOBLACIONSUFAAZA,
        ...
        pulseEnable: true,
        onFinish: () {
          _setTutorialShownPOBLACIONSUFAAZA(); // Guarda que ya se mostró
        },
  ...
  final List<TargetFocus> _initTargetsPOBLACIONSUFAAZA = [];
  ...
  void initState() {
    ...
    if (kIsWeb) {
      ...
      _SUFAAZABaseRef =
          FirebaseDatabase.instance.ref(EnvLoader.get('SUFAAZA_')!);
    } else {
      _SUFAAZARef = FirebaseDatabase.instance.ref(dotenv.env['SUFAAZA_ROWS']!);
      _SUFAAZABaseRef = FirebaseDatabase.instance.ref(dotenv.env['SUFAAZA_']!);
    }
    _initTargetsPOBLACIONSUFAAZA.add(
      TargetFocus(
        identify: "SelectPiscina",
        keyTarget: _SelectPiscPOBLACIONSUFAAZA,
    ...
    _initTargetsPOBLACIONSUFAAZA.add(
      TargetFocus(
        identify: "NumberLancesSUFAAZA",
        keyTarget: _NumberLancesSUFAAZA,
        alignSkip: Alignment.bottomRight,
        radius: 20,
        shape: ShapeLightFocus.RRect,
        contents: [
    ...
    _initTargetsPOBLACIONSUFAAZA.add(
      TargetFocus(
        ...
        keyTarget: _DiametroAtarrayaPOBLACIONSUFAAZA,
        alignSkip: Alignment.bottomRight,
        radius: 20,
        shape: ShapeLightFocus.RRect,
        contents: [
    ...
    _initTargetsPOBLACIONSUFAAZA.add(
      TargetFocus(
        ...
        keyTarget: _ResultFinalPoblacionSUFAAZA,
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _timerPOBLACIONSUFAAZA = Timer(const Duration(seconds: 1), () {
        if (mounted) {
          showTutorialPOBLACIONSUFAAZA();
        }
      });
    });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        controller: _scrollControllerSUFAAZA,
        child: Padding(
          padding: const EdgeInsets.all(8.0),
          child: Center(
            child: Column(
              children: [
                ...
                const SizedBox(height: 10),
                Padding(
                  key: _ResultFinalPoblacionSUFAAZA,
                  padding: const EdgeInsets.all(8.0),
                  child: InkWell(
                    onTap: () {
  ...
  Widget _buildHectSelect(List<String> itemsHect, String selectedItemHect,
    ...
    return Column(
      ...
      children: [
        Text(
          key: _SelectPiscPOBLACIONSUFAAZA,
          titleHect,
          style: const TextStyle(
            fontSize: 18,
            fontWeight: FontWeight.bold,
  ...
  Widget CustomTextFieldDiametro(
    ...
    return Container(
      ...
      child: Padding(
        padding: const EdgeInsets.all(8.0),
        child: TextField(
          key: _DiametroAtarrayaPOBLACIONSUFAAZA,
          controller: controller,
          keyboardType: keyboardType,
          readOnly: isReadOnly,
          decoration: InputDecoration(
