...
import 'package:azaktilza/Presentation/Utils/getBackgroundColor.dart';
import 'package:azaktilza/Presentation/Utils/gradient_colors.dart';
import 'package:azaktilza/Presentation/views/POBLACION/Functions/exportarAExcel.dart';
import 'package:azaktilza/Presentation/views/POBLACION/Functions/exportarAPDF.dart';
import 'package:azaktilza/Presentation/views/POBLACION/Functions/share_exported_file.dart';
import 'package:azaktilza/Presentation/views/POBLACION/Widgets/resultado_data.dart';
import 'package:azaktilza/Presentation/views/POBLACION/Widgets/resultado_data_source.dart';
import 'package:azaktilza/Presentation/views/POBLACION/Widgets/tabla_dinamica_resultado.dart';
import 'package:azaktilza/env_loader.dart';
import 'package:tutorial_coach_mark/tutorial_coach_mark.dart';

class Poblacion_FERTIAGRO_Screen extends StatefulWidget {
  const Poblacion_FERTIAGRO_Screen({Key? key}) : super(key: key);

  @override
  _Poblacion_FERTIAGRO_Screen_State createState() =>
      _Poblacion_FERTIAGRO_Screen_State();
}

class _Poblacion_FERTIAGRO_Screen_State
    extends State<Poblacion_FERTIAGRO_Screen> {
  late final DatabaseReference _FERTIAGRORef;

  late final DatabaseReference _FERTIAGROBaseRef;
  ...
  String _selectedFinca = 'FERTIAGRO';
  int _currentPage = 0;
  final int _itemsPerPage = 6;
  Timer? _timerPOBLACIONFERTIAGRO;
  ...
  void _fetchTerrenos() async {
    try {
      DatabaseEvent event = await _FERTIAGROBaseRef.once();
  ...
  void _updateTableRowsResult() {
    ...
    if (kIsWeb) {
      ...
      resultRef = FirebaseDatabase.instance
          .ref('${dotenv.env['POBLACION']}/$_selectedFinca');
    }
    ...
    Future.delayed(const Duration(milliseconds: 300), () {
      if (isTableVisibleResult) {
        _scrollControllerFERTIAGRO.animateTo(
          _scrollControllerFERTIAGRO.position.maxScrollExtent,
...
// coach mark
  final ScrollController _scrollControllerFERTIAGRO = ScrollController();
  final GlobalKey _SelectPiscPOBLACIONFERTIAGRO = GlobalKey();
  final GlobalKey _BeforePoblacion = GlobalKey();
  final GlobalKey _AfterPoblacion = GlobalKey();
  final GlobalKey _DiametroAtarrayaPOBLACIONFERTIAGRO = GlobalKey();
  final GlobalKey _NumberLancesFERTIAGRO = GlobalKey();
  final GlobalKey _ResultFinalPoblacionFERTIAGRO = GlobalKey();
  final GlobalKey _resetTutorialPoblation = GlobalKey();
  TutorialCoachMark? tutorialCoachMarkPOBLACIONFERTIAGRO;
  ...
  void dispose() {
    ...
    _timerPOBLACIONFERTIAGRO?.cancel(); // Cancela el timer si está activo
    tutorialCoachMarkPOBLACIONFERTIAGRO?.finish();
    super.dispose();
  }

  String get tutorial_mostrado_FERTIAGRO_POBLACION =>
      "tutorial_mostrado_FERTIAGRO_POBLACION";

  // Verifica si el tutorial ya fue mostrado
  Future<bool> _shouldShowTutorialPOBLACIONFERTIAGRO() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getBool(tutorial_mostrado_FERTIAGRO_POBLACION) ??
        true; // Si no existe, se muestra
  }

// Guarda que el tutorial ya se mostró
  Future<void> _setTutorialShownPOBLACIONFERTIAGRO() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(tutorial_mostrado_FERTIAGRO_POBLACION, false);
  }

// Restablece el tutorial desde ajustes si el usuario lo quiere repetir
  Future<void> resetTutorialPOBLACIONFERTIAGRO() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(tutorial_mostrado_FERTIAGRO_POBLACION, true);
  }

  // Muestra el tutorial solo si no se ha mostrado antes
  void showTutorialPOBLACIONFERTIAGRO() async {
    if (await _shouldShowTutorialPOBLACIONFERTIAGRO()) {
      tutorialCoachMarkPOBLACIONFERTIAGRO = TutorialCoachMark(
        targets: _initTargetsPOBLACIONFERTIAGRO,
        ...
        pulseEnable: true,
        onFinish: () {
          _setTutorialShownPOBLACIONFERTIAGRO(); // Guarda que ya se mostró
        },
  ...
  final List<TargetFocus> _initTargetsPOBLACIONFERTIAGRO = [];
  ...
  void initState() {
    ...
    if (kIsWeb) {
      _FERTIAGRORef =
          FirebaseDatabase.instance.ref(EnvLoader.get('FERTIAGRO_ROWS')!);
      _FERTIAGROBaseRef =
          FirebaseDatabase.instance.ref(EnvLoader.get('FERTIAGRO_')!);
    } else {
      _FERTIAGRORef =
          FirebaseDatabase.instance.ref(dotenv.env['FERTIAGRO_ROWS']!);
      _FERTIAGROBaseRef =
          FirebaseDatabase.instance.ref(dotenv.env['FERTIAGRO_']!);
    }
    _initTargetsPOBLACIONFERTIAGRO.add(
      TargetFocus(
        identify: "SelectPiscina",
        keyTarget: _SelectPiscPOBLACIONFERTIAGRO,
    ...
    _initTargetsPOBLACIONFERTIAGRO.add(
      TargetFocus(
        identify: "NumberLancesFERTIAGRO",
        keyTarget: _NumberLancesFERTIAGRO,
        alignSkip: Alignment.bottomRight,
        radius: 20,
        shape: ShapeLightFocus.RRect,
        contents: [
    ...
    _initTargetsPOBLACIONFERTIAGRO.add(
      TargetFocus(
        ...
        keyTarget: _DiametroAtarrayaPOBLACIONFERTIAGRO,
        alignSkip: Alignment.bottomRight,
        radius: 20,
        shape: ShapeLightFocus.RRect,
        contents: [
    ...
    _initTargetsPOBLACIONFERTIAGRO.add(
      TargetFocus(
        ...
        keyTarget: _ResultFinalPoblacionFERTIAGRO,
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _timerPOBLACIONFERTIAGRO = Timer(const Duration(seconds: 1), () {
        if (mounted) {
          showTutorialPOBLACIONFERTIAGRO();
        }
      });
    });
  ...
  void _loadHectareasPiscinas(String finca) async {
    final snapshot = await _FERTIAGRORef.get();
    if (snapshot.exists) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        controller: _scrollControllerFERTIAGRO,
        child: Padding(
          padding: const EdgeInsets.all(8.0),
          child: Center(
            child: Column(
              children: [
                ...
                const SizedBox(height: 10),
                Padding(
                  key: _ResultFinalPoblacionFERTIAGRO,
                  padding: const EdgeInsets.all(8.0),
                  child: InkWell(
                    onTap: () {
      ...
      floatingActionButton: SpeedDial(
        ...
        children: [
          ...
          SpeedDialChild(
            ...
            onTap: () async {
              await resetTutorialPOBLACIONFERTIAGRO();
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text(
  ...
  Widget _buildHectSelect(List<String> itemsHect, String selectedItemHect,
    ...
    return Column(
      ...
      children: [
        Text(
          key: _SelectPiscPOBLACIONFERTIAGRO,
          titleHect,
          style: const TextStyle(
            fontSize: 18,
            fontWeight: FontWeight.bold,
  ...
  Widget CustomTextFieldDiametro(
    ...
    return Container(
      ...
      child: Padding(
        padding: const EdgeInsets.all(8.0),
        child: TextField(
          key: _DiametroAtarrayaPOBLACIONFERTIAGRO,
          controller: controller,
          keyboardType: keyboardType,
          readOnly: isReadOnly,
          decoration: InputDecoration(
