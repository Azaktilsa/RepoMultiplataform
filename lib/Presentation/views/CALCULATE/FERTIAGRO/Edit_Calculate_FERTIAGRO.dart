...
import 'package:azaktilza/Presentation/views/CALCULATE/Widgets/Edit/resumen_tablas.dart';
import 'package:azaktilza/env_loader.dart';

class Edit_Calculate_FERTIAGRO extends StatefulWidget {
  final String id;

  const Edit_Calculate_FERTIAGRO({super.key, required this.id});

  @override
  State<Edit_Calculate_FERTIAGRO> createState() =>
      _Edit_Calculate_FERTIAGROState();
}

class _Edit_Calculate_FERTIAGROState extends State<Edit_Calculate_FERTIAGRO> {
  ...
  void _saveDataAlimentation(BuildContext context) async {
    try {
      ...
      if (kIsWeb) {
        ...
        dbRef = FirebaseDatabase.instance.ref(
            '${dotenv.env['RESULT_ALIMENTATION']}/$_selectedFinca/${widget.id}');
      }
