...
class DatoSUFAAZA_Screen extends StatefulWidget {
  const DatoSUFAAZA_Screen({Key? key}) : super(key: key);

  @override
  _DatoSUFAAZA_ScreenState createState() => _DatoSUFAAZA_ScreenState();
}

class _DatoSUFAAZA_ScreenState extends State<DatoSUFAAZA_Screen> {
  ...
  void _loadPiscinas() async {
    final data = await Dato_Logic.fetchDataFromRef(_SUFAAZARef);
    setState(() {
...
// Función para mostrar mensajes con SnackBar
  ...
  void initState() {
    ...
    if (SUFAAZAData.isNotEmpty) {
      final defaultHectareas =
          '${SUFAAZAData[0]['Hectareas']}_${SUFAAZAData[0]['Piscinas']}';
              ...
              children: [
                HectSelectWidget(
                  ...
                  onSelect: (tipo, item, isSelected) {
                    ...
                    final terreno = SUFAAZAData.firstWhere(
                      (element) =>
