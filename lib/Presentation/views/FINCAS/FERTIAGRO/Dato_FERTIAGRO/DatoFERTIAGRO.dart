// ignore_for_file: unnecessary_null_comparison, unused_local_variable, use_build_context_synchronously, library_private_types_in_public_api
...
class DatoFERTIAGRO_Screen extends StatefulWidget {
  const DatoFERTIAGRO_Screen({Key? key}) : super(key: key);

  @override
  _DatoFERTIAGRO_ScreenState createState() => _DatoFERTIAGRO_ScreenState();
}

class _DatoFERTIAGRO_ScreenState extends State<DatoFERTIAGRO_Screen> {
  String? selectedHectareas;
  String? selectedPiscinas;
  String? selectedGramos;

  List<Map<String, dynamic>> FERTIAGROData = [];
  List<Map<String, dynamic>> rendimientoData = [];

  List<String> piscinasOptions_FERTIAGRO = [];
  ...
  final String _selectedFinca = 'FERTIAGRO';
  ...
  void _fetchData() async {
    try {
      final FERTIAGROSnapshot = await _FERTIAGRORef.get();
      final rendimientoSnapshot = await _rendimientoRef.get();

      if (FERTIAGROSnapshot.exists) {
        setState(() {
          if (FERTIAGROSnapshot.value is List) {
            FERTIAGROData = (FERTIAGROSnapshot.value as List)
                .map((e) => Map<String, dynamic>.from(e as Map))
                .toList();
          } else if (FERTIAGROSnapshot.value is Map) {
            FERTIAGROData = (FERTIAGROSnapshot.value as Map)
                .values
                .map((e) => Map<String, dynamic>.from(e as Map))
                .toList();
          }
          ...
          piscinasOptions_FERTIAGRO =
              FERTIAGROData.map((e) => e['Piscinas'].toString()).toList();
          if (piscinasOptions_FERTIAGRO.isNotEmpty) {
            selectedPiscinas = piscinasOptions_FERTIAGRO.first;
  ...
  void _updateHectareasForPiscina(String piscina) {
    final matchingPiscina = FERTIAGROData.firstWhere(
  ...
  void _loadPiscinas() async {
    final data = await Dato_Logic.fetchDataFromRef(_FERTIAGRORef);
    setState(() {
...
// Función para mostrar mensajes con SnackBar
  ...
  void initState() {
    ...
    if (FERTIAGROData.isNotEmpty) {
      final defaultHectareas =
          '${FERTIAGROData[0]['Hectareas']}_${FERTIAGROData[0]['Piscinas']}';
              ...
              children: [
                HectSelectWidget(
                  ...
                  onSelect: (tipo, item, isSelected) {
                    ...
                    final terreno = FERTIAGROData.firstWhere(
                      (element) =>
